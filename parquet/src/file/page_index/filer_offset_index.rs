    ...
    pub(crate) fn calculate_offset_range(&self, row_group_offset: i64) -> OffsetRange {
        ...
        if page_count > 0 {
            ...
            for i in 1..page_count {
                ...
                if current_offset + current_length as i64 == offset {
...
mod tests {
    ...
    fn test_without_dictionary() {
        ...
        //
        let total_row = 40;
        let offset_index_c0 = vec![
            // add col0 page0, offset start at 0.
            PageLocation::new(0, 10, 0),
            // add col0 page1
            PageLocation::new(10, 10, 10),
            // add col0 page2
            PageLocation::new(20, 10, 20),
            // add col0 page3
            PageLocation::new(30, 10, 30),
        ];

        let offset_index_c1 = vec![
            // add col1 page0
            PageLocation::new(0, 10, 0),
            // add col1 page1
            PageLocation::new(10, 10, 20),
        ];

        let mut selected_rows = RowRanges::new_empty();
        //filter apply on col0, get result p1
        let selected_range = Range::new(10, 19);
        selected_rows.add(selected_range);

        let c0_offset_index =
            FilterOffsetIndex::try_new(&offset_index_c0, &selected_rows, total_row);
        let (start_list, length_list) = c0_offset_index.calculate_offset_range(0);
        assert_eq!(start_list.len(), 1);
        assert_eq!(length_list.len(), 1);
        // result should be col0 page1 offset
        assert_eq!(start_list[0], 10);
        assert_eq!(length_list[0], 10);

        let c1_offset_index =
            FilterOffsetIndex::try_new(&offset_index_c1, &selected_rows, total_row);
        let (start_list, length_list) = c1_offset_index.calculate_offset_range(0);
        // result should be col1 page0 offset
        assert_eq!(start_list[0], 0);
        assert_eq!(length_list[0], 10);
    }

    #[test]
    fn test_with_dictionary() {
        //  *In col 0 there is a dictionary page0 (dictionary page must be the first one in column chunk)
        //  *      ┌──────┬──────┬
        //  *      │ p0(d)│      │
        //  *      │══════│      │
        //  *   0  │  p1  │      │
        ...
        let total_row = 40;
        let offset_index_c0 = vec![
            // add col0 page0, offset start at `10`, because of the dictionary page
            PageLocation::new(10, 10, 0),
            // add col0 page1
            PageLocation::new(20, 10, 10),
            // add col0 page2
            PageLocation::new(30, 10, 20),
            // add col0 page3
            PageLocation::new(40, 10, 30),
        ];
        ...
        //TEST1 -> filter apply on col1, get result p0
        let mut selected_rows = RowRanges::new_empty();
        let selected_range = Range::new(0, 19);
        selected_rows.add(selected_range);

        let c0_offset_index =
            FilterOffsetIndex::try_new(&offset_index_c0, &selected_rows, total_row);
        let (start_list, length_list) = c0_offset_index.calculate_offset_range(0);
        // it should return 3 values, but in `calculate_offset_range` we can merge p2,p3 to one offset to reduce seek effect.
        assert_eq!(start_list.len(), 2);
        assert_eq!(length_list.len(), 2);
        // result should be col0 page0(dictionary)
        assert_eq!(start_list[0], 0);
        assert_eq!(length_list[0], 10);

        // page1 and page2(data) merged offset
        assert_eq!(start_list[1], 10);
        assert_eq!(length_list[1], 20);
        ...
        //TEST2 -> filter apply on col1, get result p1
        let mut selected_rows = RowRanges::new_empty();
        let selected_range = Range::new(20, 39);
        selected_rows.add(selected_range);
        ...
        // page3 and page4(data) merged offset
        assert_eq!(start_list[1], 30);
        assert_eq!(length_list[1], 20);

        let c1_offset_index =
            FilterOffsetIndex::try_new(&offset_index_c1, &selected_rows, total_row);
        let (start_list, length_list) = c1_offset_index.calculate_offset_range(0);
        // result should be col1 page1 offset
        assert_eq!(start_list[0], 10);
        assert_eq!(length_list[0], 10);
    }
}
